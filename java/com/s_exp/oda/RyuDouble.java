...
//
// Adapted for oda from https://github.com/ulfjack/ryu (RyuDouble.java):
// ROUND_EVEN only, writes digits directly into a byte buffer instead of
// materializing a String, debug scaffolding removed.
...
/**
 * Ryu shortest-representation double printing, Double.toString-compatible
 * formatting (scientific notation iff exponent outside [-3, 7), at least
 * two significant digits). Callers must reserve at least 24 bytes.
 */
final class RyuDouble {
                ...
                for (int k = 0; k < 4; k++) {
                    if (k == 0) {
                        POW5_INV_SPLIT[i][k] = inv
                                .shiftRight((3 - k) * POW5_INV_QUARTER_BITCOUNT).intValueExact();
                    } else {
                        POW5_INV_SPLIT[i][k] = inv
                                .shiftRight((3 - k) * POW5_INV_QUARTER_BITCOUNT).and(invMask).intValueExact();
                    }
                }
            }
        }
    ...
    /**
     * Writes the shortest decimal representation of a finite double into
     * result at index; returns the new index. Requires 24 bytes headroom.
     */
    static int write(byte[] result, int index, double value) {
        // Step 1: Decode the floating point number, unify normalized and
        // subnormal cases; trivial cases first.
        ...
        // Double.toString semantics: scientific notation iff outside [-3, 7).
        ...
        if (dmIsTrailingZeros || dvIsTrailingZeros) {
            while (dp / 10 > dm / 10) {
                if ((dp < 100) && scientificNotation) {
                    // at least two digits, per Double.toString semantics
                    break;
                }
        ...
        if (scientificNotation) {
            ...
            // Java spec formatting for values in [1E-3, 1E7).
            if (exp < 0) {
    ...
    /**
     * Computes the high digits of m * 5^p / 10^q = m * 5^(p - q) / 2^q
     * = m * 5^i / 2^j.
     */
    ...
    /**
     * Computes the high digits of m / 5^i / 2^j such that the result is
     * accurate to at least 9 decimal digits.
     */
